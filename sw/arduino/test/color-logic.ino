/*------------------------------------------------------------------------------
 ...
 This is a software prototype of the logic that will be later implemented on hardware.
...
final float CYCLE_TIME_IN_HOURS =   12.0/3600; //<---- change this
...
float get_hours_since_midnight(){
  ...
  int crnt_hr  = ths_crnt_hour();
  int crnt_min = ths_crnt_minute();
  int crnt_sec = ths_crnt_second();
  ...
  float  sec_since_midnight   
    = (SEC_IN_MIN * (crnt_hr * MIN_IN_HR + crnt_min) + 
       crnt_sec + millis / 1000);
